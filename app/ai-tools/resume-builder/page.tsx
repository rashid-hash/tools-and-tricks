...
export default function ResumeBuilderTool() {
  ...
  const [style, setStyle] = useState("Professional & ATS-Friendly");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const aiPrompt = `You are an elite Executive Resume Writer and HR Expert. Your task is to rewrite the provided background information into a highly professional, ATS-optimized Resume/CV format.
      ...
      Instructions:
      1. Create a compelling "Professional Summary" (3-4 lines).
      2. Extract and organize "Core Skills" into a clean bulleted list.
      3. Format the experience section into highly impactful, results-driven bullet points starting with strong action verbs.
      4. Format clearly using Markdown (bolding, headers, bullet points).
      5. Include placeholders like [Your Name], [Contact Info], [Dates] where needed.
      6. If the input is in Bengali, write the resume sections in standard Bengali (keeping technical terms in English). Otherwise, write entirely in English.
      7. Start directly with the resume structure. Do not include introductory filler.`;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-xs font-bold text-indigo-700 uppercase tracking-widest mb-4">
              <FileText size={14} className="text-indigo-500" /> Career Growth
            </div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-[#111827] tracking-tight flex items-center gap-3">
              AI <span className="text-indigo-600">Resume Builder</span>
            </h1>
          </div>
          <p className="text-slate-500 text-sm font-medium max-w-sm">
            আপনার অভিজ্ঞতা ও স্কিলগুলো দিন এবং সেকেন্ডের মধ্যে একটি প্রফেশনাল ATS-Friendly সিভি তৈরি করুন।
          </p>
        </div>
      </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl p-6 rounded-[32px] shadow-[0_20px_60px_rgb(15,23,42,0.06)] border border-slate-200/60">
            ...
            <div className="mb-6">
              <label className="text-[12px] font-bold text-slate-500 uppercase tracking-widest block mb-3 flex items-center gap-2">
                <Briefcase size={16} className="text-indigo-500"/> Target Job Role
              </label>
              <input
                type="text"
                value={targetRole}
                onChange={(e) => setTargetRole(e.target.value)}
                placeholder="কোন পদের জন্য সিভি বানাচ্ছেন? (যেমন: UI/UX Designer)"
            ...
            <div className="mb-6">
              <label className="text-[12px] font-bold text-slate-500 uppercase tracking-widest block mb-3 flex items-center gap-2">
                <GraduationCap size={16} className="text-indigo-500"/> Experience & Background
              </label>
              <textarea
                value={experience}
                onChange={(e) => setExperience(e.target.value)}
                placeholder="কোথায় কাজ করেছেন, কী কী পারেন এবং শিক্ষাগত যোগ্যতা সম্পর্কে সংক্ষেপে লিখুন..."
                className="w-full h-40 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-2xl p-4 text-sm font-medium text-[#111827] outline-none transition-all resize-none"
              />
            </div>

            <div className="mb-8">
              <label className="text-[12px] font-bold text-slate-500 uppercase tracking-widest block mb-3 flex items-center gap-2">
                <Settings2 size={16} className="text-indigo-500"/> Resume Format
              </label>
              <select 
                value={style}
              ...
              >
                <option value="Professional & ATS-Friendly">Professional & ATS-Friendly</option>
                <option value="Modern & Creative">Modern & Creative</option>
                <option value="Executive (For Senior Roles)">Executive (For Senior Roles)</option>
              </select>
            </div>

            <button 
              onClick={handleGenerate}
              disabled={!targetRole.trim() || !experience.trim() || isGenerating}
          ...
          <div className="bg-[#1E293B] border-b border-slate-700 px-6 py-4 flex justify-between items-center z-10">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">
                {isGenerating ? "Formatting content..." : "Resume Draft"}
              </span>
            </div>
            ...
            {output ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-60">
                ...
                <p className="font-medium text-center max-w-sm mt-4">
                  কোন রোলের জন্য সিভি বানাতে চান এবং আপনার কাজের অভিজ্ঞতা লিখে Generate বাটনে ক্লিক করুন। 
                </p>
              </div>
            )}
          </div>
